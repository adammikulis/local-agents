...
layout(set = 0, binding = 5, std430) restrict readonly buffer Updraft { float vel_up[]; };
...
void main() {
	...
	float lift = clamp(vel_up[g] / max(params.updraft_ref, 1e-30), 0.0, 1.0);
	...
	int below = la_step(nbr, g, normalize(gv));
	if (below < 0) {
		return;
	}
	charge[g] += dq;
