...
var metabolism: float = 0.5           # THIS FRAME's resting cost (Q10-scaled by water temperature)
# --- BODY MASS (LACreatureBodyMass), the same physiology a land animal runs on ---------------------------
...
var basal_metabolism: float = 0.5     # resting cost at the Q10 reference temperature, before thermal scaling
var bite_rate: float = 1.0            # mass units a mouth can process per second
var thermal_strategy: String = "ectotherm"   # a fish is an ectotherm; the cetacean declares otherwise
var thermal_speed_mult: float = 1.0   # a cold ectotherm swims slowly (chill coma), it does not burn more
...
func setup(_terrain, _mat_field, _config: Dictionary) -> void:
	...
	thermal_strategy = LACreatureBodyMass.strategy(config)
	structural_mass = LACreatureBodyMass.structural(config)
	max_energy = LACreatureBodyMass.reserve(config)
	energy = max_energy
	basal_metabolism = LACreatureBodyMass.basal_rate(config)
	metabolism = basal_metabolism
	bite_rate = LACreatureBodyMass.bite_rate(config)
...
func _physics_process(delta: float) -> void:
	...
	#
	# The burn is thermal too. A fish is an ECTOTHERM: its rate follows the Q10 curve on the water temperature
	# around it, which means cold water genuinely slows a fish's metabolism instead of costing it energy.
	var thermal_cause: String = ""
	if material != null and material.has_method("temp_at"):
		thermal_cause = LACreatureThermal.tick(self, material.temp_at(pos), delta)
	...
	if energy <= 0.0:
		...
		return
	if thermal_cause != "":
		die(thermal_cause)                        # past its critical thermal minimum or maximum
		return
	...
	# it lowers every rate it has, and the Q10 curve that set the metabolism above sets this from the same read.
	var step_len: float = speed * thermal_speed_mult * delta
