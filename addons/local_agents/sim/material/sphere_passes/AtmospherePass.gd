...
func _setup(bufs: Dictionary, cc: int) -> void:
	...
	for p in 2:
		...
		# moisture[live] -> the private scratch. Atmospheric water has no settled phase here (precipitation
		# is the next stage), so the deposit binding is the scratch and is never written.
		_transport_set[p] = _uset(_transport_pipe,
			Tracer.bindings(bufs, moisture[p], _moist_buf, _moist_buf))
...
func dispatch(rd: RenderingDevice, cl: int, parity: int, ctx: Dictionary, cc: int, groups: int) -> void:
	...
	var pc_tr: PackedByteArray = Tracer.push_constant(cc, _ctx_depth(ctx), Tracer.courant(cell_size),
			Tracer.gas_settle_v(LAPhysical.MOLAR_MASS_WATER_KG_MOL), MOISTURE_DIFFUSE, false,
			_ctx_core_radius(ctx), cell_size)
