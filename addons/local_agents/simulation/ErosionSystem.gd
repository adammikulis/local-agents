...
func apply_geomorph_delta(
	...
	voxel_world["column_index_by_tile"] = _build_column_index(columns)
	...
	voxel_world["block_rows_by_chunk"] = _build_chunk_row_index(voxel_world.get("block_rows", []), chunk_size)
	...
	voxel_world["surface_y_buffer"] = _build_surface_y_buffer(columns, width, height)
...
func _build_column_index(columns: Array) -> Dictionary:
...
func _build_surface_y_buffer(columns: Array, width: int, height: int) -> PackedInt32Array:
