...
String field_name_from_handle(const Variant &handle_variant) {
    if (handle_variant.get_type() != Variant::DICTIONARY) {
        if (handle_variant.get_type() == Variant::STRING || handle_variant.get_type() == Variant::STRING_NAME) {
            const String token = String(handle_variant).strip_edges();
            if (token.begins_with("field::")) {
                return token.substr(7, token.length() - 7);
            }
            return token;
        }
        return String();
...
Array read_field_evolution_buffer(const Dictionary &field_buffers, const Dictionary &frame_inputs, const Array &field_handles, const String &requested_field) {
    Array aliases = build_field_source_aliases(requested_field);
    for (int64_t i = 0; i < field_handles.size(); i += 1) {
        ...
        if (!handle_field.is_empty() && handle_field == requested_field) {
            if (!source_field.is_empty()) {
                append_scalar_if_nonempty(aliases, source_field);
            }
        }
    }

    for (int64_t i = 0; i < aliases.size(); i += 1) {
        const String alias = String(aliases[i]);
        if (alias.is_empty()) {
            continue;
        }
        Array values = to_numeric_array(field_buffers.get(alias, Variant()));
        if (!values.is_empty()) {
            return values;
        }
    }
    ...
    for (int64_t i = 0; i < aliases.size(); i += 1) {
        ...
        Array values = to_numeric_array(frame_inputs.get(alias, Variant()));
        if (!values.is_empty()) {
            return values;
        }
        Array field_values = to_numeric_array(frame_inputs.get(alias + String("_field"), Variant()));
...
Dictionary run_field_buffer_evolution(
    ...
    Array mass = read_field_evolution_buffer(field_buffers, frame_inputs, field_handles, String("mass"));
    Array pressure = read_field_evolution_buffer(field_buffers, frame_inputs, field_handles, String("pressure"));
    Array temperature = read_field_evolution_buffer(field_buffers, frame_inputs, field_handles, String("temperature"));
    Array velocity = read_field_evolution_buffer(field_buffers, frame_inputs, field_handles, String("velocity"));
    Array density = read_field_evolution_buffer(field_buffers, frame_inputs, field_handles, String("density"));
