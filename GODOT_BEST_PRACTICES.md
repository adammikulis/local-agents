...
### 2026-07-09: known issue — `rc=134` SIGABRT at process exit under Metal/MoltenVK (engine-internal, benign)

- Symptom: a windowed Metal run (`scripts/run_sim_offscreen.sh … VoxelWorld.tscn -- --run-frames=N`) prints `SIM_REPORT={…}` normally, then aborts on the way out with `libc++abi: terminating due to uncaught exception … recursive_mutex lock failed: Invalid argument` and exits `rc=134`. This has been hit by multiple independent sessions.
- Where it fires: the backtrace is entirely engine/OS — `-[NSApplication terminate:]` → `NSNotificationCenter postNotificationName:` → a MoltenVK (`mvk…`) termination-observer callback → `std::recursive_mutex` on an already-destroyed mutex. There are **no GDScript frames** in the stack: it happens during `NSApplication` teardown, after `SceneTree` has shut down and after our `_exit_tree`/`dispose()` have returned. It is MoltenVK's own global `VkInstance` teardown reacting to the app-termination notification, not our code freeing anything.
- What it is NOT: it is NOT caused by leaking our GPU RIDs, and it is NOT fixed by freeing them. Verified 2026-07-09 on `feature/gpu-teardown`: `LAMaterialSphereGPU3D.dispose()` + every `sphere_passes/*.gd` `dispose()` now free all buffers/uniform-sets/pipelines/shaders on their local `RenderingDevice` before `_rd.free()`, so the RenderingDevice teardown reports **0 leaked RIDs** (baseline was 25 Shader + 25 Compute + 2 StorageBuffer leaked). With zero leaks the `rc=134` abort still fires in the identical `NSApplication terminate:` → `recursive_mutex` MoltenVK frame — proving it is downstream of, and independent of, our resource management.
- Preventative pattern / how to treat it:
  - Freeing every RID cleanly and freeing the local `RenderingDevice` while the tree is still up (`MaterialField3D._exit_tree` → `_gpu.dispose()`) is still the correct, required discipline — it removes the leaked-RID warnings and any RID-ordering crash. Keep every new sphere pass giving itself a `dispose(rd)` that frees its sets → pipelines → owned buffers → shaders (borrowed `bufs` entries are freed by the driver, never the pass).
  - Do NOT add a hacky teardown workaround (force `_exit()`/`OS.kill` before MoltenVK's observer, etc.) to mask the `rc=134`.
  - When gating a windowed Metal run, treat a `SIM_REPORT` that emitted with sane aggregates + **0 leaked-RID warnings** as the pass signal; an `rc=134` whose backtrace is the `NSApplication terminate:` → `recursive_mutex` MoltenVK frame (no GDScript frames) is this known engine-internal exit crash, not a regression. A non-`134` abort, or `134` with a GDScript frame in the stack, IS a real failure to investigate.
