...
func run_test(tree: SceneTree) -> bool:
    ...
    var oral_primary = service.record_oral_knowledge("nk_route_aria", "npc_aria", "water_route", "Aria teaches the river path.", 0.92, ["river", "hunting"], 13, {"notes": "water lore"})
    ok = ok and _assert(oral_primary.get("ok", false), "Failed to record oral knowledge for npc_aria")
    var oral_secondary = service.record_oral_knowledge("nk_route_bram", "npc_bram", "ritual_chant", "Bram learns the chant from Aria.", 0.87, ["chant"], 14)
    ok = ok and _assert(oral_secondary.get("ok", false), "Failed to record oral knowledge for npc_bram")
    ok = ok and _assert(service.link_oral_knowledge_lineage("nk_route_aria", "nk_route_bram", "npc_aria", "npc_bram", 1, 14).get("ok", false), "Failed to link oral lineage")
    var lineage_relink = service.link_oral_knowledge_lineage("nk_route_aria", "nk_route_bram", "npc_aria", "npc_bram", 1, 14)
    ok = ok and _assert(lineage_relink.get("lineage_exists", false), "Lineage should be idempotent")
    var oral_for_aria = service.get_oral_knowledge_for_npc("npc_aria", 16, 4)
    ok = ok and _assert(oral_for_aria.get("oral_knowledge", []).size() >= 1, "Oral knowledge fetch missing entries for Aria")
    var lineage_chain = service.get_oral_lineage("nk_route_bram")
    ok = ok and _assert(lineage_chain.get("lineage", []).size() >= 1, "Lineage chain missing parent knowledge")

    ok = ok and _assert(service.upsert_sacred_site("site_spring", "spring", {"x": 102.5, "y": 0.0, "z": -13.7}, 4.5, ["taboo_water"], 12, {"notes": "primary spring"}).get("ok", false), "Failed to upsert sacred site")
    var site = service.get_sacred_site("site_spring")
    ok = ok and _assert(site.get("site", {}).get("site_id", "") == "site_spring", "Fetched sacred site mismatch")

    ok = ok and _assert(service.record_ritual_event("ritual_sunrise", "site_spring", 14, ["npc_aria", "npc_bram"], {"blessing": "sun"}, {"mood": "solemn"}).get("ok", false), "Failed to record ritual event")
    var ritual_history = service.get_ritual_history_for_site("site_spring", 16, 8)
