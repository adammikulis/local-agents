...
layout(push_constant, std430) uniform Params {
	...
	float shell_m;     // metres one regolith shell stands for
	float step_s;      // real seconds one field step stands for
} params;
...
const float MAX_MASS = 1.0;           // fill fraction at which a cell's surface water is full
const float MAX_FLOW_FRAC = 0.35;     // cap on the share of a cell's soil that leaves in one step
...
const float RESIDUAL = 0.30;          // fraction of capacity held against gravity by capillarity
...
void main() {
	...
	if (params.pass_id == 0u) {
		...
		// phi for every cell, every step, before any early return; zero outside regolith.
		...
		if (is_regolith) {
			...
			if (surplus > 0.0) {
				...
				if (up >= 0 && solid[up] == 0.0) {
					// seep and spring share this outlet, so they share its remaining capacity.
	...
	// Probe: the same gather split by donor type. from_reg = donor is regolith, from_open = donor is open.
	float from_reg = 0.0;
	float from_open = 0.0;
	int nb; float sflow;
	// The slot that answers each link comes from `partner`, never from arithmetic on the slot index.
	...
	if (regolith[g] != 0.0) {
		...
		soil_out[g] = soil[g];                          // impermeable bedrock: inert, drops `inflow`
