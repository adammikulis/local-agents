...
##
## A voice turns a `LocalAgentSynthVoiceParamsResource` into a mono float buffer
...
func render(_params: LocalAgentSynthVoiceParamsResource, _sample_rate: int) -> PackedFloat32Array:
...
func render_to_stream(params: LocalAgentSynthVoiceParamsResource, sample_rate: int = DEFAULT_SAMPLE_RATE, loop: bool = false) -> AudioStreamWAV:
