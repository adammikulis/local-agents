...
func _tick_plant_seeding() -> void:
	...
	for p in plants:
		...
		var placed = _eco._place_on_surface(_eco._tangent_offset_point((p as Node3D).global_position, randf_range(-3.5, 3.5), randf_range(-3.5, 3.5)))
...
func _tick_tree_seeding() -> void:
	...
	while seeded < TREE_SEEDS_PER_TICK and guard < TREE_SEEDS_PER_TICK * 4:
		...
		seeded += 1
		var placed = _eco._place_on_surface(_eco._tangent_offset_point(parent.global_position, randf_range(-TREE_SEED_SPREAD, TREE_SEED_SPREAD), randf_range(-TREE_SEED_SPREAD, TREE_SEED_SPREAD)))
