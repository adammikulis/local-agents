...
layout(set = 0, binding = 2, std430) readonly buffer AtmospherePrecipitation { float v[]; } atmosphere_precipitation;
layout(set = 0, binding = 3, std430) readonly buffer AtmosphereCover { float v[]; } atmosphere_cover;
layout(set = 0, binding = 4, std430) readonly buffer SurfaceMoisture { float v[]; } surface_moisture;
layout(set = 0, binding = 5, std430) readonly buffer SurfaceMotionNorm { float v[]; } surface_motion_norm;
layout(set = 0, binding = 6, std430) readonly buffer SurfaceSaturation { float v[]; } surface_saturation;
layout(set = 0, binding = 7, std430) readonly buffer Activity { float v[]; } activity;
layout(set = 0, binding = 8, std430) restrict buffer SurfaceWearBudget { float v[]; } surface_wear_budget;
layout(set = 0, binding = 9, std430) restrict buffer SurfaceFracture { float v[]; } surface_fracture;
...
void main() {
	...
	float r = clamp(atmosphere_precipitation.v[idx], 0.0, 1.0);
	float c = clamp(atmosphere_cover.v[idx], 0.0, 1.0);
	float w = clamp(surface_moisture.v[idx], 0.0, 1.0);
	float f = clamp(surface_motion_norm.v[idx], 0.0, 1.0);
	float wr = clamp(surface_saturation.v[idx], 0.0, 1.0);
	...
	float atmospheric_cooling = r * 0.038 + c * 0.024;
	float t = clamp(base_t + seasonal_swing - atmospheric_cooling, 0.0, 1.0);
	...
	float frost = clamp(surface_fracture.v[idx] * 0.982 + impulse * 0.12, 0.0, 1.0);
	float slope_wear = max(0.0, (sl * 0.62 + f * 0.38) * r * 0.028 * local_dt);
	float fracture_wear = frost * (0.004 + crack * 0.006) * local_dt;
	float cumulative = surface_wear_budget.v[idx] + slope_wear + fracture_wear;
	float cycles = floor(cumulative / 0.12);
	float next_budget = cumulative - cycles * 0.12;
	float drop = max(0.0, cycles) * 0.004;

	surface_fracture.v[idx] = frost;
	temp_prev.v[idx] = t;
	surface_wear_budget.v[idx] = next_budget;
