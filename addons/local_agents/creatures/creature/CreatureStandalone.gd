...
static func setup(c, config_source, opts: Dictionary) -> void:
	...
	# Join the shared cognition group (LocalAgentCognitionScheduler's default adopt_group) so a scheduler
	# dropped into the scene wires this creature to the model with no code at all.
