...
// Units: velocity m/s, k_lat = dt_seconds / lateral_metres. Advection is v*dt/dx.
...
// Settled phase, for a tracer that has one (dust -> sediment). Unwritten when `deposit == 0`; the caller
// still binds a buffer, and it must not alias binding 0 or 1.
...
layout(push_constant, std430) uniform Params {
	...
	float k_lat;          // dt / LATERAL spacing, seconds over metres
	...
	uint offset;          // index base: channel-packed tracers pass ch*cell_count, others 0
	float decay;          // per-step fractional loss, 0 for a conserved tracer
	float lat_ref;        // the lateral spacing `k_lat` was divided by, model units
} params;
...
// Speed of cell `c` toward its lateral link `l` (0..3, i.e. neighbour slots N_LAT0+l), m/s, in its own
// tangent frame projected onto the per-link direction `ltan` carries.
...
// Settling share of cell `c` across radial slot `d`: still-air settling velocity suppressed by wind, times
// that face's Courant factor. Positive sinks, negative rises; callers split it across the two faces because
// a share is an outflow fraction and cannot be negative.
...
void main() {
	...
	// Every share this cell sends is scaled by scale_g, and every neighbour gathers that same scaled share
	// off its own reverse link, so the flux across a face is one number both ends agree on. A share is a
	// fraction of the DONOR's cell, so the credit is multiplied by vol(donor)/vol(receiver).
	float raw = raw_out(g);
	float gain = 0.0;
	for (int l = 0; l < 4; ++l) {
		...
		// The donor's own lateral index for the link back to this cell, from `link_partner`.
	...
	// Vertical inflow: the cell below blowing up into this one (advection + mixing), and the cell above
	// sending its whole downward flux — its settling plus the mixing share.
	...
	// The downward flux that meets solid ground settles in this same cell, so no volume ratio applies.
