...
func update_volumetric_cloud_transform_stage(controller: Node, rain: float, cloud: float, humidity: float, wind: Vector2, wind_speed: float) -> void:
	update_cloud_layer_transform_stage(controller, rain, cloud, humidity, wind, wind_speed)
...
func update_cloud_layer_transform_stage(controller: Node, rain: float, cloud: float, humidity: float, wind: Vector2, wind_speed: float) -> void:
	if not controller.clouds_enabled:
		return
	controller._ensure_renderer_nodes()
	apply_cloud_quality_settings(controller)
	controller._cloud_renderer.update_transform_stage(
		rain,
		cloud,
		humidity,
		wind,
		wind_speed,
		controller._transform_stage_a_field_texture,
		controller._transform_stage_a_field_world_size,
...
func ensure_transform_stage_a_field_texture(controller: Node) -> void:
	...
	var needs_recreate = (
		controller._transform_stage_a_field_image == null
		or controller._transform_stage_a_field_texture == null
		or controller._transform_stage_a_field_image.get_width() != width
		or controller._transform_stage_a_field_image.get_height() != height
	)
	controller._transform_stage_a_field_world_size = Vector2(float(width), float(height))
	if not needs_recreate:
		return
	controller._transform_stage_a_field_image = Image.create(width, height, false, Image.FORMAT_RGBA8)
	controller._transform_stage_a_field_image.fill(Color(0.0, 0.0, 0.0, 1.0))
	controller._transform_stage_a_field_texture = ImageTexture.create_from_image(controller._transform_stage_a_field_image)
	controller._transform_stage_a_field_cache.resize(width * height)
	for i in range(controller._transform_stage_a_field_cache.size()):
		controller._transform_stage_a_field_cache[i] = -1
	controller._transform_stage_a_field_last_avg_pack = -1
...
func ensure_transform_stage_d_field_texture(controller: Node) -> void:
	var width = maxi(1, int(controller._generation_snapshot.get("width", 1)))
	var height = maxi(1, int(controller._generation_snapshot.get("height", 1)))
	var needs_recreate = (
		controller._transform_stage_d_field_image == null
		or controller._transform_stage_d_field_texture == null
		or controller._transform_stage_d_field_image.get_width() != width
		or controller._transform_stage_d_field_image.get_height() != height
	)
	if not needs_recreate:
		return
	controller._transform_stage_d_field_image = Image.create(width, height, false, Image.FORMAT_RGBA8)
	controller._transform_stage_d_field_image.fill(Color(0.0, 0.0, 0.0, 1.0))
	controller._transform_stage_d_field_texture = ImageTexture.create_from_image(controller._transform_stage_d_field_image)
	controller._transform_stage_d_field_cache.resize(width * height)
	for i in range(controller._transform_stage_d_field_cache.size()):
		controller._transform_stage_d_field_cache[i] = -1
	controller._transform_stage_d_field_last_tick = -1
...
func refresh_surface_state_from_generation(controller: Node) -> void:
	...
	for y in range(height):
		for x in range(width):
			...
			var pack = pack_transform_stage_color(c)
...
func update_surface_state_texture(controller: Node, atmosphere_state_snapshot: Dictionary) -> void:
	ensure_surface_field_texture(controller)
	if controller._surface_field_image == null or controller._surface_field_texture == null:
		return
	var tick = int(atmosphere_state_snapshot.get("tick", -1))
	...
	var width = controller._surface_field_image.get_width()
	var height = controller._surface_field_image.get_height()
	var rows: Array = atmosphere_state_snapshot.get("rows", [])
	var buffers: Dictionary = atmosphere_state_snapshot.get("buffers", {})
	var stage_a_rain: PackedFloat32Array = buffers.get("rain", PackedFloat32Array())
	var stage_a_humidity: PackedFloat32Array = buffers.get("humidity", PackedFloat32Array())
	var use_buffers = stage_a_rain.size() == width * height and stage_a_humidity.size() == width * height
	...
	var avg_humidity = clampf(float(atmosphere_state_snapshot.get("avg_humidity", 0.0)), 0.0, 1.0)
	var dirty = 0
	...
	for offset in range(budget):
		...
		if use_buffers:
			...
			humidity = clampf(float(stage_a_humidity[idx]), 0.0, 1.0)
		else:
		...
		var surface_wear = clampf(prev.a * 0.986 + rain * flow * 0.022 + wet * flow * 0.009, 0.0, 1.0)
		var next = Color(wet, snow, flow, surface_wear)
		var pack = pack_transform_stage_color(next)
...
func update_transform_stage_d_field_texture(controller: Node, exposure_state_snapshot: Dictionary) -> void:
	ensure_transform_stage_d_field_texture(controller)
	if controller._transform_stage_d_field_image == null or controller._transform_stage_d_field_texture == null:
		return
	var tick = int(exposure_state_snapshot.get("tick", -1))
	if tick >= 0 and (tick % maxi(1, controller.transform_stage_d_texture_update_interval_ticks)) != 0:
		return
	if tick == controller._transform_stage_d_field_last_tick:
		return
	controller._transform_stage_d_field_last_tick = tick
	var width = controller._transform_stage_d_field_image.get_width()
	var height = controller._transform_stage_d_field_image.get_height()
	var avg_absorbed = clampf(float(exposure_state_snapshot.get("avg_insolation", 0.0)), 0.0, 1.0)
	var avg_uv = clampf(float(exposure_state_snapshot.get("avg_uv_index", 0.0)) / 2.0, 0.0, 1.0)
	var avg_heat = clampf(float(exposure_state_snapshot.get("avg_heat_load", 0.0)) / 1.5, 0.0, 1.0)
	var avg_growth = clampf(float(exposure_state_snapshot.get("avg_growth_factor", 0.0)), 0.0, 1.0)
	var buffers: Dictionary = exposure_state_snapshot.get("buffers", {})
	...
	var rows: Array = exposure_state_snapshot.get("rows", [])
	var dirty = 0
	if use_buffers:
		var budget = _texture_budget_for_controller(controller, expected)
		var start = clampi(controller._transform_stage_d_field_update_cursor, 0, maxi(0, expected - 1))
		for offset in range(budget):
			...
			var pack = pack_transform_stage_color(c)
			if controller._transform_stage_d_field_cache[i] == pack:
				continue
			controller._transform_stage_d_field_cache[i] = pack
			controller._transform_stage_d_field_image.set_pixel(x, y, c)
			dirty += 1
		controller._transform_stage_d_field_update_cursor = (start + budget) % maxi(1, expected)
	elif rows.size() < expected:
		var fill = Color(avg_absorbed, avg_uv, avg_heat, avg_growth)
		var fill_pack = pack_transform_stage_color(fill)
		for i in range(controller._transform_stage_d_field_cache.size()):
			if controller._transform_stage_d_field_cache[i] == fill_pack:
				continue
			controller._transform_stage_d_field_cache[i] = fill_pack
			var x = i % width
			var y = i / width
			controller._transform_stage_d_field_image.set_pixel(x, y, fill)
			dirty += 1
	if not use_buffers:
		for row_variant in rows:
			if not (row_variant is Dictionary):
			...
			var pack = pack_transform_stage_color(c)
			var idx = y * width + x
			if idx < 0 or idx >= controller._transform_stage_d_field_cache.size():
				continue
			if controller._transform_stage_d_field_cache[idx] == pack:
				continue
			controller._transform_stage_d_field_cache[idx] = pack
			controller._transform_stage_d_field_image.set_pixel(x, y, c)
			dirty += 1
	if dirty > 0:
		controller._transform_stage_d_field_texture.update(controller._transform_stage_d_field_image)

func update_transform_stage_a_field_texture(controller: Node, atmosphere_state_snapshot: Dictionary) -> void:
	ensure_transform_stage_a_field_texture(controller)
	if controller._transform_stage_a_field_image == null or controller._transform_stage_a_field_texture == null:
		return
	var tick = int(atmosphere_state_snapshot.get("tick", -1))
	if tick >= 0 and (tick % maxi(1, controller.transform_stage_a_texture_update_interval_ticks)) != 0:
		return
	var width = controller._transform_stage_a_field_image.get_width()
	var height = controller._transform_stage_a_field_image.get_height()
	var avg_cloud = clampf(float(atmosphere_state_snapshot.get("avg_cloud_cover", 0.0)), 0.0, 1.0)
	var avg_rain = clampf(float(atmosphere_state_snapshot.get("avg_rain_intensity", 0.0)), 0.0, 1.0)
	var avg_humidity = clampf(float(atmosphere_state_snapshot.get("avg_humidity", 0.0)), 0.0, 1.0)
	var avg_fog = clampf(float(atmosphere_state_snapshot.get("avg_fog_intensity", 0.0)), 0.0, 1.0)
	var fill_color = Color(avg_cloud, avg_rain, avg_humidity, avg_fog)
	var avg_pack = pack_transform_stage_color(fill_color)
	var buffers: Dictionary = atmosphere_state_snapshot.get("buffers", {})
	...
	if use_buffers:
		...
		var start = clampi(controller._transform_stage_a_field_update_cursor, 0, maxi(0, expected_cells - 1))
		for offset in range(budget):
			...
			var pack = pack_transform_stage_color(pixel)
			if controller._transform_stage_a_field_cache[i] == pack:
				continue
			controller._transform_stage_a_field_cache[i] = pack
			var x = i % width
			var y = i / width
			controller._transform_stage_a_field_image.set_pixel(x, y, pixel)
			dirty_count += 1
		controller._transform_stage_a_field_update_cursor = (start + budget) % maxi(1, expected_cells)
	elif rows.size() < expected_cells and avg_pack != controller._transform_stage_a_field_last_avg_pack:
		fill_transform_stage_a_field(controller, fill_color, avg_pack)
		dirty_count = expected_cells
	controller._transform_stage_a_field_last_avg_pack = avg_pack
	if not use_buffers:
		for row_variant in rows:
			if not (row_variant is Dictionary):
				continue
			...
			var pack = pack_transform_stage_color(pixel)
			var idx = y * width + x
			if idx < 0 or idx >= controller._transform_stage_a_field_cache.size():
				continue
			if controller._transform_stage_a_field_cache[idx] == pack:
				continue
			controller._transform_stage_a_field_cache[idx] = pack
			controller._transform_stage_a_field_image.set_pixel(x, y, pixel)
			dirty_count += 1
	if dirty_count <= 0:
		return
	controller._transform_stage_a_field_texture.update(controller._transform_stage_a_field_image)

func pack_transform_stage_color(c: Color) -> int:
...
func fill_transform_stage_a_field(controller: Node, color: Color, pack: int) -> void:
	if controller._transform_stage_a_field_image == null:
		return
	controller._transform_stage_a_field_image.fill(color)
	for i in range(controller._transform_stage_a_field_cache.size()):
		controller._transform_stage_a_field_cache[i] = pack
