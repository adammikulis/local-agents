...
# which are full-field scans, so at low FPS the wall-clock SAMPLE_HZ gate alone would fire every frame. The
# graph is a background readout, so a coarser update is invisible.
