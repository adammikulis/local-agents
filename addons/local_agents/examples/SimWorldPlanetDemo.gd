...
## SimWorldPlanetDemo — a whole cubed-sphere PLANET with a living ecology, with NO game shell: no HUD,
## menus, disasters, save system or streamer.
##
...
##     Switch World Type to Flat and the same node builds a ground plane instead — no godot_voxel needed.
##   - Camera3D frames the planet (Far is raised to see past it); DirectionalLight3D lights the view.
##     SimWorld also builds its own child Sun, which is the light the field's solar pass is driven by —
##     that one is simulation, this one is presentation.
