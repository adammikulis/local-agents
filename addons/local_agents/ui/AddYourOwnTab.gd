...
class_name LocalAgentAddYourOwnTab
...
var _download_manager: LocalAgentModelDownloadManager = null
...
func _on_download_progress(_model_id: String, received: int, total: int, speed: float, eta: float) -> void:
	var received_text: String = LocalAgentModelDownloadManager.format_bytes(received)
	var total_text: String = LocalAgentModelDownloadManager.format_bytes(total) if total > 0 else "?"
	_download_status.text = "%s / %s · %s · %s" % [received_text, total_text, LocalAgentModelDownloadManager.format_speed(speed), LocalAgentModelDownloadManager.format_eta(eta)]
