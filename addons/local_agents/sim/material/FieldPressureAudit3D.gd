...
## Pressure may not fall as you go DOWN: a decrease inward is the kernel contradicting gravity. A pure
## instrument -- it reads the drained mirrors and requests nothing.

## Cells whose inward neighbour holds a LOWER pressure, and the worst such drop in pascals.
static func inversions(f) -> Dictionary:
	var out: Dictionary = {"pressure_inversions": 0, "pressure_audited": 0}
