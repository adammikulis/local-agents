...
layout(set = 0, binding = 1, std430) restrict buffer Scratch { float scratch[]; }; // lava snapshot
layout(set = 0, binding = 2, std430) restrict buffer Temp { float temp[]; };       // temp[back], degC
layout(set = 0, binding = 41, std430) restrict buffer TempSnap { float temp_snap[]; };  // temp snapshot, degC
layout(set = 0, binding = 3, std430) restrict readonly buffer Solid { float solid[]; };
layout(set = 0, binding = 15, std430) restrict readonly buffer Neigh { int nbr[]; };  // idx*6 + slot

layout(push_constant, std430) uniform Params {
	uint cell_count;
	uint pass_id;   // 0 = write snapshots, 1 = gather/apply
	uint pad0;
	uint pad1;
} params;

// Cell-fill unit; a full cell of one phase.
...
// Buoyant up-transfer a cell contributes given its lava fill.
...
void main() {
	...
	// UP: overpressure shed into the open cell above.
	int iu = nbr[base + N_OUT];
	if (iu >= 0 && solid[iu] == 0.0) {
		out_up = buoy_up(scratch[g]);
	}
	// DOWN: overpressure the open cell below buoys up into this one, converted to this cell's fill units.
	...
	// Carry-heat: mass-weighted mix of the RETAINED fill and the arriving fill. Both temperatures come from
	// the pass-0 snapshot, so the result does not depend on which cell the GPU scheduled first.
	if (in_below > 0.0 && ib >= 0 && total > 0.0) {
		temp[g] = (kept * temp_snap[g] + in_below * temp_snap[uint(ib)]) / total;
	}
}
