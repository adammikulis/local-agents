extends RefCounted
class_name LocalAgentsSettlementGrowthSystem
...
const StructureStateResourceScript = preload("res://addons/local_agents/configuration/parameters/simulation/StructureStateResource.gd")
const SettlementAnchorResourceScript = preload("res://addons/local_agents/configuration/parameters/simulation/SettlementAnchorResource.gd")

var _config = SettlementGrowthConfigResourceScript.new()
...
func set_config(config_resource) -> void:
	if config_resource == null:
		_config = SettlementGrowthConfigResourceScript.new()
		return
...
func process_tick(tick: int, household_members: Dictionary, household_metrics: Dictionary, household_positions: Dictionary, water_snapshot: Dictionary) -> Dictionary:
	var expanded: Array = []
	var abandoned: Array = []
	...
	return {
		...
		"anchors": snapshot_anchors(),
	}

func snapshot_structures() -> Dictionary:
