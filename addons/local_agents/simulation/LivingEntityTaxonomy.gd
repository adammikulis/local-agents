...
static func normalized_path(path: Array) -> Array:
	var out: Array = []
	for item in path:
...
static func animal_path(category: String, subtype: String = "") -> Array:
	var out: Array = [ROOT_LIVING_CREATURE, KINGDOM_ANIMAL, category]
	if subtype.strip_edges() != "":
		out.append(subtype)
	return normalized_path(out)

static func plant_path(category: String, subtype: String = "") -> Array:
	var out: Array = [ROOT_LIVING_CREATURE, KINGDOM_PLANT, category]
	if subtype.strip_edges() != "":
		out.append(subtype)
	return normalized_path(out)
