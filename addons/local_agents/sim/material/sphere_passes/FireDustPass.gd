...
func dispatch(rd: RenderingDevice, cl: int, parity: int, ctx: Dictionary, cc: int, groups: int) -> void:
	...
	var pc: PackedByteArray = Tracer.push_constant(cc, _ctx_depth(ctx), Tracer.courant(cell_size),
			Tracer.dust_settle_v(), Tracer.EDDY_DIFFUSE, true, _ctx_core_radius(ctx), cell_size)
