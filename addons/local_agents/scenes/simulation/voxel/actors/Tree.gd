...
const GROUP_TREE := "tree"

const GROW_TIME := 20.0        # seconds sapling -> full size
const START_FRACTION := 0.35   # freshly planted trees are visible immediately
const TOPPLE_TIME := 1.5       # seconds to fall flat
const TOPPLE_ANGLE := 1.483529 # ~85 degrees, avoids clipping through ground
...
var _tilt := 0.0               # small resting lean of the whole tree
var _upright_basis := Basis.IDENTITY
var _topple_axis := Vector3.RIGHT
var _topple_t := 0.0
var _topple_tween_active := false
...
func setup(_terrain, _config: Dictionary = {}) -> void:
	...
	var seed_val := int(config.get("seed", randi()))
	_rng.seed = seed_val
...
func _jitter_hue(base: Color, amount: float = 0.05) -> Color:
	var h := base.h + _rng.randf_range(-amount, amount)
	var s := clampf(base.s + _rng.randf_range(-0.06, 0.06), 0.0, 1.0)
	var v := clampf(base.v + _rng.randf_range(-0.08, 0.08), 0.0, 1.0)
...
func _build_trunk() -> void:
	...
	var bottom := lerpf(0.22, 0.42, clampf(trunk_height / 7.0, 0.0, 1.0))
...
func _build_broadleaf_canopy() -> void:
	...
	var blobs := _rng.randi_range(3, 4)
	var crown := trunk_height
	var base_r := maxf(trunk_height * 0.34, 0.9)
	var mat := _make_material(canopy_color)
	for i in range(blobs):
		var blob := MeshInstance3D.new()
		blob.name = "Foliage%d" % i
		var ball := SphereMesh.new()
		var r := base_r * _rng.randf_range(0.7, 1.05)
		ball.radius = r
		ball.height = r * 2.0
...
func _build_pine_canopy() -> void:
	...
	var tiers := _rng.randi_range(3, 4)
	var mat := _make_material(canopy_color)
	var base_r := maxf(trunk_height * 0.3, 0.8)
	var tier_h := trunk_height * 0.42
	var start_y := trunk_height * 0.55
	for i in range(tiers):
		var frac := float(i) / float(tiers)
...
func _advance_topple(delta: float) -> void:
	...
	var eased := ease(_topple_t, 2.2)   # accelerate as it falls
	var fall := Basis(_topple_axis, eased * TOPPLE_ANGLE)
	transform.basis = fall * _upright_basis


func get_inspector_payload() -> Dictionary:
	var title := "%s Tree" % species.capitalize()
	var status := "toppled" if toppled else ("mature" if _grown_fraction() >= 1.0 else "growing")
	return {
		"title": title,
		"lines": [
			"Species: %s" % species,
