...
fail=0
for want in WINDOW_FLAG_NO_FOCUS WINDOW_MODE_MINIMIZED; do
	rg -q "$want" "$HELPER" || {
		echo "FAIL  $HELPER no longer sets $want, so an automated run can cover or steal focus." >&2
		fail=1
	}
done

off="$(rg -n --no-heading -g '*.gd' -e 'window_set_position' "$ROOT/addons" 2>/dev/null || true)"
if [ -n "$off" ]; then
	printf '%s\n' "$off" >&2
...
echo "$line"
mode="$(printf '%s' "$line" | rg -N -o -e '"mode":[0-9]+' | rg -N -o -e '[0-9]+')"
nofocus="$(printf '%s' "$line" | rg -N -o -e '"no_focus":(true|false)' | rg -N -o -e '(true|false)')"
# WINDOW_MODE_MINIMIZED is 1.
if [ "$mode" != "1" ] || [ "$nofocus" != "true" ]; then
	echo "FAIL  a run's window came up mode=$mode no_focus=$nofocus. It must be minimized (1) and" >&2
	echo "      unfocusable, or it covers the user's work and takes the keyboard." >&2
	exit 1
fi
echo "check_quiet_window: OK (a real run's window is minimized and cannot take focus)"
