...
func rebuild_project_graph(root_path: String = "res://", extensions: Array = DEFAULT_EXTENSIONS) -> void:
    ...
    while not dir_stack.is_empty():
        ...
        while file_name != "":
            ...
            var label: String = resource_path
            var node_id: int = _graph.upsert_node(CODE_SPACE, label, metadata)
            if node_id == -1:
                file_name = dir.get_next()
                continue
            var parent_dir: String = path
            var parent_id: int = directory_ids.get(parent_dir, -1)
            if parent_id == -1:
...
func search_code(query: String, top_k: int = 5, expand: int = 64) -> Array:
    ...
    if not runtime.has_method("embed_text"):
        return []
    var emb: Variant = runtime.call("embed_text", query, {"normalize": true})
    if emb.is_empty():
        return []
    var matches: Array[Dictionary] = _graph.search_embeddings(emb, top_k, expand)
    var results: Array = []
    for match in matches:
        if not (match is Dictionary):
            continue
        var match_row: Dictionary = match
        var node_id: int = int(match_row.get("node_id", -1))
        if node_id == -1:
            continue
...
func _ensure_directory_node(path: String, cache: Dictionary) -> int:
    if cache.has(path):
        return cache[path]
    var label: String = "dir:" + path
    var data: Dictionary = {
        "type": "directory",
        "path": path,
    }
    var node_id: int = _graph.upsert_node(DIRECTORY_SPACE, label, data)
    if node_id != -1:
        cache[path] = node_id
    return node_id
...
func _clear_code_space() -> void:
    var code_nodes: Array[Dictionary] = _graph.list_nodes(CODE_SPACE, 65536, 0)
    for row in code_nodes:
        if not (row is Dictionary):
            continue
        var code_row: Dictionary = row
        _graph.remove_node(int(code_row.get("id", -1)))
    var dir_nodes: Array[Dictionary] = _graph.list_nodes(DIRECTORY_SPACE, 65536, 0)
