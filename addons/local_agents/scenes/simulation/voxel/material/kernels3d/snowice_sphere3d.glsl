...
// conserved substance in three phases (AIRWATER in the air, WATER on the ground, SNOW frozen); the phase is
...
// MASS-CONSERVING (snow += x; airwater -= x, so H₂O total = water + airwater + snow is preserved).
//
...
// airwater[idx] → race-free. FREEZE_TEMP + the sat() curve MUST match MaterialReactions3D.gd / the atmos kernels.
...
layout(set = 0, binding = 2, std430) restrict buffer AirWater { float airwater[]; };      // settled airwater (Atmosphere back) — debited
...
const float FREEZE_TEMP = 12.5;      // below this a cold ground cell freezes its condensed airwater to snow
...
void main() {
	...
	float st = temp[idx];
	if (st < FREEZE_TEMP) {
		// DEPOSITION: freeze the CONDENSED part of the air's water (airwater over saturation) — the fog/low
		// cloud resting on this cold ground — into snow. Conserving: whatever leaves airwater arrives as snow.
		float sat = SAT_BASE * exp(SAT_TEMP_GAIN * (st - EVAP_TEMP_REF));
		float condensed = max(0.0, airwater[idx] - sat);
		if (condensed > 0.0) {
