...
func _run_chunk_bounce_and_contact_queue_test(tree: SceneTree) -> bool:
	...
	# Contact rows recorded on the controller are exposed for native dispatch sampling. The
	# queue-until-dispatch + ack + deadline invariants now live in the native core and are covered
	# by test_native_projectile_dispatch_deadline_contract.gd.
	...
	var dispatch_rows := controller.sample_voxel_dispatch_contact_rows()
	ok = _assert(dispatch_rows.size() >= 1, "Recorded projectile-contact rows should be sampleable for native voxel dispatch.") and ok
