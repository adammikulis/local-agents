...
static func display_upkeep(c, delta: float) -> float:
	...
	if gene <= 0.0:
		return 0.0
	# The cost is a MULTIPLE OF THE ANIMAL'S OWN RESTING BURN, not a flat rate. It used to be an absolute
	# 1.6/sec for every creature in the game — which is a fraction of a villager's budget and many times an
	# insect's entire body, so the ornament was free for the large and instantly lethal for the small. An
	# honest signal has to cost the SAME SHARE of the bearer's budget whatever it weighs; that is what makes
	# it comparable between suitors, and it is why the handicap principle works at every body size.
	return c.basal_metabolism * DISPLAY_UPKEEP_OVER_BASAL * gene * gene * delta  # quadratic: very bright is disproportionately costly

const DISPLAY_UPKEEP_OVER_BASAL: float = 0.9
