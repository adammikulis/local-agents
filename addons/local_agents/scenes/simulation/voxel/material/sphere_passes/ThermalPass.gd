...
func setup(rd: RenderingDevice, bufs: Dictionary, cc: int) -> void:
	...
	for p in 2:
		...
		# lava_phase: 0 = lava (BACK, in-place), 1 = temp (BACK, in-place), 2 = solid, 15 = nbr (shell-first
		# edge cooling reads each cell's 6 faces to count EXPOSED faces -> a flow's rind hardens while the core
		# stays molten and drains, leaving a lava tube).
		_lava_phase_set[p] = _make_set(rd, _lava_phase_shader, [
			[0, lava_back], [1, temp_back], [2, solid], [15, nbr]])
