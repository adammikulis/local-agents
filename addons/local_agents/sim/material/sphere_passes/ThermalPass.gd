...
func dispatch(rd: RenderingDevice, cl: int, parity: int, ctx: Dictionary, cc: int, groups: int) -> void:
	...
	var emit_pc: PackedByteArray = _lava_phase_pc(cc, dt_s, cell_size, 0)
	...
	rd.compute_list_bind_compute_pipeline(cl, _lava_phase_pipe)
	rd.compute_list_bind_uniform_set(cl, _lava_phase_set[parity], 0)
	var deposit_pc: PackedByteArray = _lava_phase_pc(cc, dt_s, cell_size, 1)
...
# lava_phase Params: { uint cell_count; float dt_s; float cell_size; uint mode; } — 16 bytes.
# mode 0 = emit, 1 = deposit. The kernel turns an exposed-face flux in W/m^2 into a temperature, so it
# needs the step's real seconds and the size.
func _lava_phase_pc(cc: int, dt_s: float, cell_size: float, mode: int) -> PackedByteArray:
	var pc: PackedByteArray = PackedByteArray()
	pc.resize(16)
	pc.encode_u32(0, cc)
	pc.encode_float(4, dt_s)
