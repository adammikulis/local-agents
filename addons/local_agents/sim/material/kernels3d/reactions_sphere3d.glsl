...
#define OVERBURDEN 20  // DERIVED driver: LITHOSTATIC pressure (Pa) of the SOLID column above. See overburden().
#define BEDROCK_BELOW 21 // DERIVED, WRITABLE: the bedrock of the SOLID cell directly beneath this open one —
...
const float KELVIN_OFFSET = 273.15;     // LAPhysical.KELVIN_OFFSET — Arrhenius needs absolute temperature
...
void main() {
	...
	for (uint r = 0u; r < params.n_records; r++) {
		...
		if (rc.rate_model == CONST_FRAC) {
			...
			float t_k = min(temp[i], BOIL_TEMP) + KELVIN_OFFSET;
