...
static func environment_tick_contract(dispatch: Dictionary, tick: int, payload: Dictionary = {}) -> Dictionary:
	var native_result_variant = dispatch.get("result", {})
	var native_result: Dictionary = native_result_variant if native_result_variant is Dictionary else {}
	...
	var counters_variant = native_result.get("counters", {})
	var counters: Dictionary = counters_variant if counters_variant is Dictionary else {}
	var payload_contacts_variant = payload.get("physics_contacts", payload.get("physics_server_contacts", []))
	var payload_contacts: Array = payload_contacts_variant if payload_contacts_variant is Array else []
	var dispatched := is_environment_stage_dispatched(dispatch)
	return {
		"tick": tick,
		"delta": maxf(0.0, float(payload.get("delta", 0.0))),
		"dispatched": dispatched,
		"status": String(native_result.get("status", "")),
		"error": String(dispatch.get("error", "")),
		"contacts_submitted": payload_contacts.size(),
		"contacts_consumed": payload_contacts.size() if dispatched else 0,
		"ops_requested": maxi(0, int(native_result.get("ops_requested", execution.get("ops_requested", 0)))),
		"ops_processed": maxi(0, int(native_result.get("ops_processed", execution.get("ops_processed", 0)))),
		"ops_requeued": maxi(0, int(native_result.get("ops_requeued", execution.get("ops_requeued", 0)))),
		...
		"dispatch_count_total": maxi(0, int(counters.get("dispatch_count_total", 0))),
		"dispatch_count_stage": maxi(0, int(counters.get("dispatch_count_stage", 0))),
	}
...
static func _aggregate_contact_inputs(rows: Array[Dictionary]) -> Dictionary:
	return NativeComputeBridgeContactNormalizationScript.aggregate_contact_inputs(rows)

static func _sort_aggregated_contact_rows(left_variant, right_variant) -> bool:
	return NativeComputeBridgeContactNormalizationScript.sort_aggregated_contact_rows(left_variant, right_variant)

static func _read_contact_velocity(raw_value) -> float:
	return NativeComputeBridgeContactNormalizationScript.read_contact_velocity(raw_value)

static func _read_float(row: Dictionary, keys: Array, fallback: float) -> float:
	return NativeComputeBridgeContactNormalizationScript.read_float(row, keys, fallback)

static func _read_vector3_from_keys(row: Dictionary, keys: Array) -> Vector3:
	return NativeComputeBridgeContactNormalizationScript.read_vector3_from_keys(row, keys)

static func _read_vector3(raw_value) -> Vector3:
	return NativeComputeBridgeContactNormalizationScript.read_vector3(raw_value)
